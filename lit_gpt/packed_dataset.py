...
class PackedDatasetBuilder(object):
    def __init__(self, outdir, prefix, chunk_size, sep_token=0, dtype="auto", vocab_size=None):
        if dtype == "auto":
            if vocab_size == 1145:
                # raise ValueError("vocab_size cannot be None when dtype='auto'")
                self._dtype = np.float32
            elif vocab_size is not None and vocab_size < 65500:
    ...
    def _norm(self,arr):
        ...
        if len(arr.shape) == 2:
            for i in range(arr.shape[-1]):
                scalings[:,i] = torch.mean(scalings[:,:(i+1)],dim = -1)
        
        else:
            for i in range(arr.shape[-1]):
                scalings[i] = torch.mean(scalings[:(i+1)])
                
        arr = arr / scalings
        
        return torch.cat([arr,scalings],dim = -1)
